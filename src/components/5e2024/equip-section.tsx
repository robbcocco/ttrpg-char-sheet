...
import { ICharacterArmor, ICharacterShield, ICharacterWeapon, loadArmors, loadShields, loadWeapons } from "@/types/5e2024/character-equip";
...
export default function EquipSection() {
    ...
    const [availableShields, setAvailableShields] = useState<ICharacterShield[]>([]);
    ...
    const onUpdateShield = (shieldData?: ICharacterShield) => {
        dispatch(characterActions.updateCharacterShield(shieldData));
    };

    useEffect(() => {
        setAvailableWeapons(loadWeapons());
        setAvailableArmors(loadArmors());
        setAvailableShields(loadShields());
    }, []);
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-6">
            ...
            {/* Add Shield */}
            <div>
                <label className="block text-sm font-medium text-gray-700">Shield</label>
                <select
                    value={`${characterEquip?.shield ? `${JSON.stringify({ backgroundName: characterEquip.shield.name, backgroundSource: characterEquip.shield.source })}` : ''}`}
                    onChange={(e) => {
                        if (!e.target.value || e.target.value == '') {
                            onUpdateShield();
                        } else {
                            const { backgroundName, backgroundSource } = JSON.parse(e.target.value);
                            const newBackground = availableShields.find(bg => bg.name == backgroundName && bg.source == backgroundSource);
                            if (newBackground) onUpdateShield(newBackground);
                        }
                    }}
                    className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                    <option value="">Select Shield</option>
                    {availableShields.map(background => (
                        <option key={`${background.name}-${background.source}`} value={`${JSON.stringify({ backgroundName: background.name, backgroundSource: background.source })}`}>{`${background.name} (${background.source})`}</option>
                    ))}
                </select>
            </div>
